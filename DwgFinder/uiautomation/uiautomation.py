...
class ValuePattern():
    ...
    def SetValue(self, value: str, waitTime: float = OPERATION_WAIT_TIME):
        """
...
class Control():
    ...
    @property
    def IsSelected(self):
        return self.GetSelectionItemPattern().IsSelected
    ...
    def Exists(self, maxSearchSeconds: float = 3, searchIntervalSeconds: float = SEARCH_INTERVAL,
               printIfNotExist: bool = False, Exception=True):
        """
        ...
        if prev and not prev._element and not prev.Exists(maxSearchSeconds, searchIntervalSeconds, Exception=Exception):
    ...
    def MoveCursorToInnerPos(self, x: int = None, y: int = None, ratioX: float = 0.5, ratioY: float = 0.5,
                             simulateMove: bool = False) -> Tuple[int, int]:
        """
    ...
    def MiddleClick(self, x: int = None, y: int = None, ratioX: float = 0.5, ratioY: float = 0.5,
                    simulateMove: bool = False, waitTime: float = OPERATION_WAIT_TIME) -> None:
        """
    ...
    def WaitProperty(self, property, value='', waitTime=TIME_OUT_SECOND):
        """
        等待控件的属性变成指定值
        property:属性类型
        value:执行的属性值
        ...
        while time.time() - start_time < waitTime:
            if property.lower() == "name" and self.Name == value:
                return True
            elif property.lower() == "text":
                if isinstance(self, ComboBoxControl) or isinstance(self, EditControl):
                    if self.GetCurrentValue() == value:
                        return True
                elif isinstance(self, PaneControl):
                    if self.Name == value:
                        return True
                else:
                    if self.GetWindowText() == value:
                        return True
            elif property == "enable":
                if value and self.IsEnabled:
                    return True
...
class CheckBoxControl(Control):
    ...
    def Set(self, state):
        if state.upper() == "OFF" and self.GetTogglePattern().ToggleState:
            self.Click()
        elif state.upper() == "ON" and not self.GetTogglePattern().ToggleState:
            self.Click()
...
class ComboBoxControl(Control):
    ...
    def Select(self, itemName: str = '', condition: Callable[[str], bool] = None, simulateMove: bool = False,
               waitTime: float = OPERATION_WAIT_TIME) -> bool:
        """
    ...
    def GetAllValue(self, simulateMove: bool = False):
        expandCollapsePattern = self.GetExpandCollapsePattern()
        if expandCollapsePattern:
            expandCollapsePattern.Expand()
        else:
            # Windows Form's ComboBoxControl doesn't support ExpandCollapsePattern
            self.Click(x=-10, ratioY=0.5, simulateMove=simulateMove)
        listControl = self.ListControl(searchDepth=1)
        datas = []
        for item, depth in WalkControl(listControl, includeTop=False):
            if isinstance(item, ListItemControl):
                datas.append(item.Name)
        SendKeys('{ESC}')
        return datas

    def Set(self, content):
        self.EditControl(foundIndex=1).Set(content)
...
class EditControl(Control):
    ...
    def GetCurrentValue(self):
        return self.GetValuePattern().Value.strip()

    def Set(self, text):
        self.GetValuePattern().SetValue(text)
...
class ListControl(Control):
    ...
    def Select(self, item, button='Left', simulateMove=False):
        list_items = self.GetChildren()
        if isinstance(item, int):
            if item > len(list_items) - 1:
                raise ValueError('index out of range')
            else:
                item = self.ListItemControl(searchDepth=1, foundIndex=item + 1)
                if item.Exists(1):
                    if scrollItemPattern := item.GetScrollItemPattern():
                        scrollItemPattern.ScrollIntoView(waitTime=0.1)
                    if button == "Right":
                        item.RightClick(simulateMove=simulateMove)
                    else:
                        item.Click(simulateMove=simulateMove)
                    return True
        elif isinstance(item, str):
            for list_item in list_items:
                if list_item.Name == item and list_item.ControlTypeName == 'ListItemControl':
                    if list_item.Exists(1):
                        if scrollItemPattern := list_item.GetScrollItemPattern():
                            scrollItemPattern.ScrollIntoView(waitTime=0.1)
                        if button == "Right":
                            list_item.RightClick(simulateMove=simulateMove)
                        else:
                            list_item.Click(simulateMove=simulateMove)
                        return True
            else:
                raise ValueError('Not Found Valid ListItem')
        return False

    def SelectRange(self, start, end):
        PressKey(Keys.VK_CONTROL)
        for i in range(start, end + 1):
            if not self.GetSubItem(i).GetSelectionItemPattern().IsSelected:
                self.GetSubItem(i).Click()
        ReleaseKey(Keys.VK_CONTROL)

        # for i in range(int(start), int(end) + 1):
        #     listitem = self.ListItemControl(foundIndex=i + 1)
        #     listitem.GetSelectionItemPattern().AddToSelection()

    def GetSelect(self) -> 'ListItemControl':
        return self.ListItemControl(Compare=lambda c, d: c.GetSelectionItemPattern().IsSelected)

    def GetAllValue(self):
        datas = []
        list_items = self.GetChildren()
        for list_item in list_items:
            if list_item.ControlTypeName == 'ListItemControl' and list_item.Exists(1):
                if scrollItemPattern := list_item.GetScrollItemPattern():
                    scrollItemPattern.ScrollIntoView(waitTime=0.1)
                datas.append(list_item.Name)
        return datas

    def GetAllItems(self):
        list_items = self.GetChildren()
        return [item for item in list_items if item.ControlTypeName == 'ListItemControl' and item.Exists(1)]

    def GetItemCount(self):
        return len(self.GetAllItems())

    def GetSubItem(self, list_item_index, subitem_index=None):
        list_item = self.ListItemControl(foundIndex=int(list_item_index) + 1)
        if subitem_index is not None:
            sub_item = list_item.Control(foundIndex=int(subitem_index) + 1, Depth=1)
        else:
            sub_item = list_item
        return sub_item
...
class MenuControl(Control):
    ...
    def Select(self, item, simulateMove=False):
        menu_items = self.GetChildren()
        if isinstance(item, int):
            if item > len(menu_items) - 1:
                raise ValueError('index out of range')
            else:
                menu_items[item].Click(simulateMove=simulateMove)
                return True
        elif isinstance(item, str):
            for menu_item in menu_items:
                if menu_item.Name == item:
                    menu_item.Click(simulateMove=simulateMove)
                    return True
            else:
                raise ValueError('Not Found Valid TabItem')

    def GetAllItem(self):
        items = []
        for item, depth in WalkControl(self, includeTop=False):
            if isinstance(item, MenuItemControl):
                items.append(item)
        return items
...
class TopLevel():
    ...
    def SetActive(self, waitTime: float = OPERATION_WAIT_TIME) -> bool:
        ...
        if self.IsTopLevel():
            ...
            ret = SetForegroundWindow(handle)  # may fail if foreground object_repository's process is not python
            time.sleep(waitTime)
            return ret
        return False
...
class ScrollBarControl(Control):
    ...
    def Set(self, value, waittime=0):
        try:
            self.GetRangeValuePattern().SetValue(int(value), waitTime=waittime)
        except:
            pass
...
class TabControl(Control):
    ...
    def Select(self, item, simulateMove=False):
        tab_items = self.GetChildren()
        if isinstance(item, int):
            if item > len(tab_items) - 1:
                raise ValueError('index out of range')
            else:
                tab_items[item].Click(simulateMove=simulateMove)
                return True
        elif isinstance(item, str):
            for tab_item in tab_items:
                if tab_item.Name == item:
                    tab_item.Click(simulateMove=simulateMove)
                    return True
            else:
                raise ValueError('Not Found Valid TabItem')

    def GetSection(self):
        return self.GetSelectionPattern().GetSelection()[0]
...
class ToolBarControl(Control):
    ...
    def Press(self, index):
        self.ButtonControl(foundIndex=int(index), Depth=1).Click()
...
class TreeControl(Control):
    ...
    def ExpandAll(self, items):
        items = items.split(";")
        for item, depth in WalkControl(self, includeTop=False):
            if isinstance(item, TreeItemControl) and item.Name in items:
    ...
    def GetAllItems(self):
        items = []
        for item, depth in WalkControl(self, includeTop=False):
            if isinstance(item, TreeItemControl):
                items.append(item)
        return items

    def Select(self, items, button='Left', simulateMove=False, x=None, y=None):
        if isinstance(items, int):
            select_item = self.TreeItemControl(foundIndex=1)
        else:
            items = items.split(";")
            select_item = self
            for item in items:
                child = select_item.TreeItemControl(Name=item)
                if scrollItemPattern := child.GetScrollItemPattern():
                    scrollItemPattern.ScrollIntoView(waitTime=0.1)
                ...
                select_item = child
        if button == "Right":
            select_item.RightClick(x, y, simulateMove=simulateMove)
        else:
            select_item.Click(x, y, simulateMove=simulateMove)
        return True

    def GetSelction(self):
        return self.GetSelectionPattern().GetSelection()
...
class TreeItemControl(Control):
    ...
    def Select(self, item, button='Left', simulateMove=False):
        tree_items = self.GetChildren()
        if isinstance(item, int):
            if item > len(tree_items) - 1:
                raise ValueError('index out of range')
            else:
                item = self.TreeItemControl(searchDepth=1, foundIndex=item + 1)
                if item.Exists(1):
                    if scrollItemPattern := item.GetScrollItemPattern():
                        scrollItemPattern.ScrollIntoView(waitTime=0.1)
                    if button == "Right":
            ...
            for tree_item in tree_items:
                if tree_item.Name == item and tree_item.ControlTypeName == 'TreeItemControl':
                    if tree_item.Exists(1):
                        if scrollItemPattern := tree_item.GetScrollItemPattern():
                            scrollItemPattern.ScrollIntoView(waitTime=0.1)
                        if button == "Right":
                            tree_item.RightClick(simulateMove=simulateMove)
                        else:
                            tree_item.Click(simulateMove=simulateMove)
                        return True
            else:
                raise ValueError('Not Found Valid TreeItem')
        return False
...
class WindowControl(Control, TopLevel):
    ...
    def Close(self, WaitTime=0.5):
        win32gui.PostMessage(self.NativeWindowHandle, win32con.WM_CLOSE, 0, 0)
        WaitTime and time.sleep(WaitTime)
